...
其中 `weight[r] = marginal[r]`（见下方"显存代价"）。
...
```
weight[r] = marginal[r]
          = 2 × retained[r]       # q/k 双通路激活保留
          + max(fwd[r], bwd[r])   # 峰值取大
          + 2 × param[r]          # 参数 + SGD momentum buffer
```

`base_cost = overhead_mb`（模型参数 + CUDA 运行时 + LN 共享激活）不参与 MILP 的 per-scale 选择，作为 knapsack 的固定 offset。
...
后续实验通过 `--scale-memory-cache`（`SCALE_MEMORY_CACHE=1`）加载。

---

## Coverage Sensitivity 实验

独立脚本 `scripts/coverage_sensitivity.sh`，对每个 `(cov, alpha)` 组合使用 `--coverage-strength 1` 并行启动：
