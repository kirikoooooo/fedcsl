...
def _collect_scale_memory_table(server_model, sample_data, device):
    ...
    for branch_name in ['shapelets_euclidean', 'shapelets_cosine', 'shapelets_cross_correlation']:
        ...
        if branch is not None:
            branch._bw_profiling_enabled = True
    ...
    for branch_name in ['shapelets_euclidean', 'shapelets_cosine', 'shapelets_cross_correlation']:
        branch = getattr(server_model, branch_name, None)
        if branch is not None:
            branch._bw_profiling_enabled = False
    for pname, p in server_model.named_parameters():
        p.requires_grad_(saved_grad_flags.get(pname, False))
...
def train(dataset="", seed=42, T=0.1, l=1e-2, ls=1.0, alpha=0.5, batch_size=8, to_cuda=True,
    ...
    for round in range(start_round, numRound):
        ...
        if round == 0 and use_scale_split_comm:
            ...
            if scale_costs is not None and client_scale_plans is not None:
                ...
                for cid, scales in enumerate(client_scale_plans):
                    ...
                    acc_tag = f"累加={acc:.1f}MB"
                    acc_vs_budget = ""
                    if c_budget is not None and c_budget > 0:
                        acc_vs_budget = f" (累加/预算={acc/c_budget*100:.0f}%)"
                    per_client_lines.append(
                        f"c{cid}: scales={sorted(scales)} | {acc_tag} | {budget_tag}{acc_vs_budget}"
                    )
                cov_line = (
                    "\n[spilter-memory-budget] per-client 显存累加（基于服务端采集数据）:\n  "
        ...
        # ── L1-A+: update cum_loss / cum_total ──
        for idx in range(numClient):
