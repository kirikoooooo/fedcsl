...
\title{FedCSL-Spilter: 面向非 IID 多变量时间序列的周期--效率感知联邦 Shapelet 表征学习}
...
\begin{abstract}
多变量时间序列广泛存在于人体活动识别、工业监测、医疗诊断与设备运维等场景中。这类数据通常天然分散在不同终端、机构或边缘节点，既难以集中汇聚，又普遍存在标签稀缺、分布异构和隐私敏感等问题。传统集中式时间序列表征学习方法在拥有完整数据时能够取得较好性能，但在联邦环境下往往面临客户端漂移严重、跨客户端表示不一致以及训练效率受限等挑战。现有联邦时间序列方法多侧重监督分类、参数平均或系统调度，而对可解释的多尺度 shapelet 子模型分发、聚合与效率分析仍然不足。

针对上述问题，本文提出一种面向非独立同分布多变量时间序列的联邦 shapelet 学习框架 FedCSL-Spilter。该方法以 TimeCSL/CSL 的多尺度 shapelet 表征学习思想为基础，在联邦训练中引入三类关键机制。首先，在表示学习层面，本文在本地实例级对比学习目标之上，引入局部--全局联合对比学习与联合知识蒸馏机制，使客户端模型在保留本地适应性的同时，与全局语义空间保持一致。其次，在尺度分发层面，本文设计 Spilter 尺度分发机制：每个客户端默认只训练 $m$ 个尺度，其中 $b$ 个为基础尺度，另外 $a=m-b$ 个由服务器依据尺度参数量、滑窗计算量与当前尺度效用进行效率感知补发，并仅下发/回收被选中的尺度参数，按尺度粒度完成聚合。再次，在多尺度建模层面，本文结合 STFT 与 ACF 构造周期感知的尺度权重，并将该统计信号同时用于损失加权和额外尺度分配。

本文进一步从优化与系统角度分析了所提方法的合理性。我们指出，多尺度 shapelet 的参数量与计算量随着尺度长度增长而显著变化，因此简单的随机补发会造成训练预算浪费；效率感知的尺度分发能够在保持表示能力的同时降低通信和计算开销。实验部分基于 UEA 多变量时间序列分类数据集以及若干扩展数据集进行评估，并从总体性能、消融实验、非 IID 强度、收敛趋势和可解释性分析等多个方面验证方法有效性。结果表明，FedCSL-Spilter 在下游分类性能、表示稳定性和系统效率方面均优于多种联邦表示学习基线与异构模型分发基线。
\end{abstract}

\noindent\textbf{关键词：} 联邦学习；多变量时间序列；Shapelet；多尺度拆分；效率感知分发；对比学习；知识蒸馏
...
把集中式的 TimeCSL/CSL 直接迁移到联邦环境并不是一件简单的事情。首先，联邦场景下不同客户端数据往往呈现严重的非独立同分布特性，不同用户、设备和场景对应的数据统计特征、类别分布和时间模式存在明显差异。若仍采用纯粹的局部训练加参数平均，客户端模型很容易过拟合其本地模式，导致全局模型在表示空间中出现漂移和不稳定现象。其次，时间序列本身具有复杂的多尺度结构，不同数据集甚至同一数据集中的不同样本，其主导模式可能出现在不同的时间尺度上。如果在联邦环境下仍对各尺度一视同仁，则会降低 shapelet 学习的针对性。再次，联邦训练中的“模型异构”同样会体现在尺度异构上：某些尺度的参数量更大、滑窗计算更重，另一些尺度则更轻更快。如果所有尺度都被随机补发或平均对待，系统很容易把预算浪费在低效尺度上。

已有研究从不同角度探讨了异构联邦模型如何适应系统资源。HeteroFL 通过宽度裁剪将同一全局模型转化为不同宽度的本地子模型\cite{heterofl}；FjORD 利用 ordered dropout 提取嵌套子模型，并结合自蒸馏提升训练稳定性\cite{fjord}；DepthFL 通过深度裁剪缓解宽度裁剪在聚合时的参数不匹配问题\cite{depthfl}；ScaleFL 同时在宽度与深度两个方向上做资源自适应裁剪\cite{scalefl}；InclusiveFL 和 AdaptiveFL 强调在强资源异构环境下不能放弃弱设备，而要通过大小模型协同和知识蒸馏共享信息\cite{inclusivefl,adaptivefl}。FlexFL 则把 APoZ-guided flexible pruning、adaptive local pruning 与 self-KD 结合起来，在不确定场景下构造更合适的异构子模型\cite{flexfl}。这些工作表明，模型分发和结构裁剪本身就是联邦效率的关键变量，但它们大多围绕通用 CNN/Transformer 展开，尚未显式考虑多尺度 shapelet 的尺度成本差异与周期效用差异。

基于此，本文提出一种面向非 IID 多变量时间序列的联邦 shapelet 学习与 Spilter 尺度分发框架 FedCSL-Spilter。本文的核心思想是：以 TimeCSL/CSL 的多尺度 shapelet 对比学习框架为骨干，在联邦环境下同时处理“如何学习可解释表示”和“如何按系统效率分发尺度”两个关键问题。具体而言，本文首先保留本地实例对比学习，并引入局部--全局联合对比学习与联合知识蒸馏，使客户端模型在保留本地适应性的同时，与全局语义空间保持一致。随后，本文围绕 Spilter 设计尺度分发机制：服务器先为每个客户端分配 $b$ 个基础尺度，再根据尺度参数量、计算量和当前尺度效用补充分配 $a=m-b$ 个额外尺度，使每个客户端每轮最终训练 $m$ 个尺度。被分发的尺度参数会按尺度粒度下发、回收与聚合，因此模型更新不再是全量传输，而是只围绕被激活尺度流动。为了让模型更好地适应不同数据集的频谱与周期结构，本文还基于 STFT 和 ACF 设计周期感知加权机制，对不同 shapelet 尺度进行动态调节，并把该统计信号复用到额外尺度的分发决策中。

综上，本文试图回答以下几个问题：第一，如何将可解释的 shapelet 表征学习扩展到联邦非 IID 环境；第二，如何围绕尺度成本而不是全模型成本构造更有效的联邦分发机制；第三，如何通过局部--全局多尺度知识对齐提升全局表示空间的一致性；第四，如何从理论与实验两个层面解释周期感知加权、尺度分发与分片聚合的有效性。

本文主要贡献概括如下：
\begin{enumerate}[leftmargin=2.5em]
    \item 提出一种面向联邦多变量时间序列的 shapelet 表征学习与 Spilter 尺度分发一体化框架，在保留 TimeCSL/CSL 可解释性的基础上，实现联邦环境下的协同表征学习。
    ...
    \item 在尺度分发层面，给出基于尺度参数量和滑窗计算量的效率感知补发策略，并与随机补发、固定核心尺度和全尺度训练等策略进行比较。
...
\subsection{异构联邦模型与尺度级分发}

在真实联邦环境中，由于带宽、设备异构性和大规模客户端数量的限制，服务器通常无法假设所有客户端都能承担同样规模的模型训练。Oort 和 FedCS 类方法表明，统计效用、设备速度、带宽与训练时延共同决定了联邦系统的 time-to-accuracy 表现\cite{oort,fedcsadaptive}。本文不再展开客户端采样问题，而是沿用这种系统效用视角，进一步关注在给定客户端参与集合下，如何把不同成本的尺度子模型更有效地分发给客户端。
...
上述工作说明，联邦系统效率不只取决于客户端是否参与，也取决于下发模型的结构粒度和资源匹配程度。与 CNN 中“通道/层”的裁剪不同，FedCSL 的天然结构单元是 shapelet 尺度：每个尺度对应一组 shapelets 及其匹配算子，不同尺度具有不同长度、不同滑窗次数和不同参数规模。因此，本文将异构模型分发具体化为\textbf{尺度级分发}：服务器不必每轮向每个客户端发送完整的 $R$ 尺度模型，而是选择 $m$ 个对该客户端更有价值、且系统代价更合适的尺度子模型进行局部训练和回传。
...
\begin{enumerate}[leftmargin=2.5em]
    \item 服务器初始化全局 shapelet 编码器，并为所有客户端预计算周期感知尺度效用分数；
    \item 服务器对每个客户端生成 Spilter 尺度计划：$b$ 个基础尺度由本地短/长尺度效用确定，$a=m-b$ 个额外尺度由系统效率感知规则补发；
    \item 客户端只接收被分配尺度的参数，在本地进行尺度级 shapelet 对比学习，并引入局部--全局联合蒸馏与尺度级蒸馏；
    \item 各客户端仅上传被训练尺度的参数更新；
    \item 服务器按尺度收集客户端更新，对每个尺度分别执行样本量加权聚合，未被本轮上传的尺度保持服务器原值；
    \item 周期感知模块根据 STFT 与 ACF 统计动态调整多尺度损失权重，并为下一次尺度计划提供效用信号；
...
\begin{figure}[H] % 放在正文引用位置附近
    ...
    \caption{FedCSL-Spilter 整体框架图。上半部分为服务器端：Scale Planner 根据客户端尺度效用、尺度参数量和滑窗计算量生成尺度计划 $\mathcal{R}_k$，只向客户端下发被选中的尺度参数；Scale-wise Aggregator 按尺度聚合上传更新，得到全局 Shapelet 编码器 $w_g$，下游以 SVM 做表示评测。下半部分为客户端 $k$ 的本地训练：多变量时间序列经数据增强形成两种视图 $x_q,x_k$，仅送入被激活的 $m$ 个尺度子模型；特征对齐节点聚合本地与全局特征，驱动局部对比损失 $\mathcal{L}_{local}$、局部--全局联合对比 $\mathcal{L}_{joint\text{-}CL}$ 与联合蒸馏 $\mathcal{L}_{joint\text{-}KD}$；右下 Period-Aware Local Weighting Module 由 STFT（短--中尺度）与 ACF（中--长尺度）两分支产生尺度权重 $\bm{\pi}$，同时反馈到多尺度损失和额外尺度分发。}
    \label{fig:framework}
\end{figure}
...
\subsection{Spilter 尺度分发与效率感知补发}
\label{sec:spilter}
...
\end{equation}
其中 $\theta^{(\mathrm{shared})}$ 表示投影、归一化或分类头等非尺度参数。Spilter 的目标是在每轮训练中仅激活一个尺度子集 $\mathcal{R}_k\subseteq\{1,\ldots,R\}$，从而让客户端 $k$ 只接收、训练和上传 $\{\theta^{(r)}:r\in\mathcal{R}_k\}$。在当前实现中，服务器从 $R$ 个候选尺度中为每个客户端激活 $m$ 个尺度，并满足 $m<R$。

\paragraph{基础尺度选择。}
服务器首先为每个客户端计算周期感知尺度分数 $\mathbf{s}_k=(s_{k,1},\ldots,s_{k,R})$，该分数由第~\ref{sec:period-score} 节的 STFT/ACF 双分支得到。为了避免只选择短尺度或只选择长尺度，先把尺度集合划分为短--中尺度组 $\mathcal{G}_1=\{1,\ldots,R/2\}$ 与中长--长尺度组 $\mathcal{G}_2=\{R/2+1,\ldots,R\}$，并分别取一个高分尺度：
\begin{equation}
\mathcal{B}_k=
\left\{
...
\right\}.
\end{equation}
这些尺度作为客户端的基础尺度，近似保证每个客户端同时接触局部模式和长周期模式。

\paragraph{尺度系统成本。}
不同尺度的系统代价并不相同。长尺度 shapelet 的参数量更大，且匹配时需要在更长窗口上计算距离、余弦相似度或互相关。记第 $r$ 个尺度的通信成本为
\begin{equation}
...
\paragraph{效率感知额外尺度补发。}
在已有基础尺度 $\mathcal{B}_k$ 后，服务器还需要补发 $m-|\mathcal{B}_k|$ 个额外尺度。旧版本 Spilter 可把这些额外尺度随机发放，并通过计数约束尽量保持全局均匀；但随机补发没有考虑不同尺度的参数量和计算效率，可能把额外预算分给高成本、低收益尺度。本文借鉴异构联邦中“按设备资源生成合适子模型”的思想\cite{heterofl,scalefl,flexfl}，把额外尺度选择改为效率感知贪心过程。设 $c_r$ 为当前已被分配到尺度 $r$ 的客户端数量，$\bar{c}$ 为理想平均覆盖数，则客户端 $k$ 在候选尺度 $r$ 上的补发得分为
\begin{equation}
\Phi_{k,r}=
...
\end{equation}
第一项表示单位系统成本带来的尺度收益，第二项惩罚已被过多客户端覆盖的尺度，从而避免所有客户端都选择同一批轻量尺度。每次选择
\begin{equation}
r^\star=\arg\max_{r\notin \mathcal{R}_k}\Phi_{k,r},
\end{equation}
并更新 $\mathcal{R}_k\leftarrow \mathcal{R}_k\cup\{r^\star\}$ 与 $c_{r^\star}\leftarrow c_{r^\star}+1$，直到 $|\mathcal{R}_k|=m$。当忽略 $\kappa_r$ 并将第一项替换为随机数时，可得到 Spilter-random 消融版本；当取 $\mathcal{R}_k=\{1,\ldots,R\}$ 时，则退化为全尺度训练。

\subsection{分片通信与尺度级聚合}

Spilter 不上传完整客户端模型，而是按尺度分片通信。第 $t$ 轮服务器向客户端 $k$ 下发
\begin{equation}
...
\end{equation}
其中 $O(KR)$ 为尺度索引和计划元数据，远小于参数传输。若所有客户端训练完整 $R$ 尺度模型，则通信量为
\begin{equation}
\mathcal{C}_{\mathrm{full}}(t)
=2K\sum_{r=1}^{R}B_r.
\end{equation}
当各尺度参数量近似相等时，$\mathcal{C}_{\mathrm{split}}/\mathcal{C}_{\mathrm{full}}\approx m/R$；当效率感知策略减少长尺度高成本分片的无效补发时，该比例会进一步下降。
...
\end{equation}
其中 $E$ 为本地 epoch 数。由于 $F_r$ 随 $\ell_r$ 和滑窗匹配次数变化，计算节省并不只是尺度数目的一半，而取决于被补发尺度的成本组合。
...
\label{alg:fedcsl}
\begin{algorithmic}[1]
\State 初始化全局模型 $w_g^{(0)}$，尺度覆盖计数 $\mathbf{c}\leftarrow\mathbf{0}$
\State 为每个客户端计算周期感知尺度分数 $\mathbf{s}_k$
\State 根据式~\eqref{eq:spilter-score} 生成尺度计划 $\mathcal{R}_k$
\For{$t=0,1,\dots,T-1$}
    \For{每个客户端 $k$ \textbf{in parallel}}
        \State 接收尺度参数 $\Theta_k^\downarrow(t)=\{\theta_g^{(r,t)}:r\in\mathcal{R}_k\}$
        \State 构造两种增强视图并计算本地对比损失
        \State 仅在 $\mathcal{R}_k$ 上计算 joint CL/KD 与 scale CL/KD
...
本节从表示信息、联邦漂移和系统效率三个角度解释 FedCSL-Spilter 的设计。本文不试图证明当前启发式尺度计划达到闭式全局最优，而是在温和假设下说明：全尺度 shapelet 训练存在信息冗余和系统拖尾；基础尺度保证每个客户端的最低信息完整性；效率感知补发是在有限通信与计算预算下对有效多尺度信息进行近似选择。
...
\end{equation}
由于时间序列在相近窗口长度上通常具有连续性，邻近尺度之间的响应相关性较高，因而常出现 $\Delta_{j\mid i}\approx 0$ 的边际收益递减。直接下发全尺度模型会提升 $I(X;Z)$ 和本地模型容量，却不一定显著提升 $I(Z;Y)$；在每个客户端样本有限且分布非 IID 的联邦场景中，过大的本地容量还可能放大局部过拟合和客户端漂移。由此，Spilter 的尺度子集可视为一种结构化正则化：它限制单个客户端每轮可拟合的尺度空间，同时通过跨客户端不同补充尺度形成多视图集成。

为了避免子集训练导致弱客户端只学习局部片段或强客户端只学习长周期片段，FedCSL-Spilter 设置基础尺度集合 $\mathcal{B}_k$。记短尺度基础表征为 $Z_{\mathrm{short}}$，长周期基础表征为 $Z_{\mathrm{long}}$。短尺度主要覆盖高频局部形态，长尺度或 ACF 驱动尺度主要覆盖低频周期结构，二者在频域和时间跨度上具有互补性。根据互信息链式法则，
\begin{equation}
I(Z_{\mathrm{short}},Z_{\mathrm{long}};Y)
=I(Z_{\mathrm{short}};Y)
 + I(Z_{\mathrm{long}};Y\mid Z_{\mathrm{short}}).
\end{equation}
当两类尺度给定标签后的冗余较低，即 $I(Z_{\mathrm{short}};Z_{\mathrm{long}}\mid Y)$ 较小时，第二项不会被短尺度完全解释。于是基础尺度为每个客户端提供了一个表征能力下界：即使客户端只激活少量尺度，其梯度也同时包含局部形态和长周期语义，降低随机子集分发中表示坍塌和跨客户端特征不对齐的风险。
...
在 Spilter 中，客户端 $k$ 只训练尺度子集 $\mathcal{R}_k$。若尺度 $r$ 的有效收益为 $u_{k,r}$，系统成本为 $c_r$，则理想尺度计划可抽象为预算约束信息选择问题：
\begin{equation}
...
\end{equation}
式~\eqref{eq:spilter-score} 用 $\widehat{s}_{k,r}$ 近似 $u_{k,r}$，用 $\kappa_r$ 近似 $c_r$，从而按单位成本收益 $\widehat{s}_{k,r}/(\kappa_r+\varepsilon)$ 贪心补发额外尺度。若集合效用
\begin{equation}
...
\end{equation}
则按收益--成本比选择尺度可视为预算约束子模优化的贪心近似。该假设与多尺度 shapelet 的冗余现象相符：少量跨度互补的尺度通常能带来主要收益，继续加入相邻尺度的边际收益逐渐下降。
...
全尺度联邦 shapelet 学习的系统瓶颈来自同步训练中的最慢客户端。设客户端 $k$ 的计算速度为 $p_k$，其被分配尺度集合的计算负载为
\begin{equation}
...
\end{equation}
若所有客户端都训练完整 $R$ 个尺度，则慢设备的 $C_k$ 与强设备相同；当 $p_k$ 很小时，式~\eqref{eq:round-makespan} 会由该客户端主导。Spilter 将完整负载替换为尺度子集负载，并通过 $\kappa_r$ 惩罚高成本尺度，本质上是在近似求解
\begin{equation}
\min_{\{\mathcal{R}_k\}_{k=1}^{K}}
\max_k
\left(
\frac{\sum_{r\in\mathcal{R}_k}F_r}{p_k}
T_k^{\mathrm{comm}}
\right),
\quad
\mathrm{s.t.}\quad
...
\end{equation}
它不改变客户端选择问题，而是在给定参与客户端上降低被分发子模型的负载方差，从而改善 time-to-accuracy。
...
\end{equation}
其中 $\Delta_{\mathrm{renorm}}$ 为已选尺度重新归一化引入的误差。基础尺度和周期评分的作用正是减小高权重尺度被遗漏的概率。
...
\end{equation}
当长尺度更重，而 Spilter 倾向于补发单位收益--成本比更高的尺度时，$\sum_{r\in\mathcal{R}_k}F_r$ 会低于随机补发的期望成本，从而得到更大的 $G_{\mathcal{F}}(t)$。综上，FedCSL-Spilter 的统一目标可概括为
\begin{equation}
...
\end{equation}
其中 $\mathcal{B}_k\subseteq\mathcal{R}_k$ 且 $|\mathcal{R}_k|=m$。该形式体现了本文方法的核心折中：在非 IID 联邦环境中，优先保留和传递最稳定、最有价值、且最符合系统预算的多尺度语义信息。
...
第二类是\textbf{FedAvg + 本文 shapelet 骨干}的结构化基线。该组方法保持本文的多尺度 shapelet 编码器和相同下游 SVM 评估协议，但仅采用 FedAvg 式参数平均，不引入局部--全局 joint CL/KD、尺度级 CL/KD、周期感知加权和 Spilter 效率感知尺度分发。该设置用于隔离“shapelet 表示本身”与“本文联邦对齐和尺度分发机制”的贡献：若 FedAvg+Shapelet 已优于通用时序骨干，说明可解释 shapelet 表示适合该任务；若 FedCSL-Spilter 进一步优于 FedAvg+Shapelet，则说明本文的多尺度蒸馏、周期权重和效率感知分发确实带来了额外收益。除主实验外，本文仍保留 Full-scale FedCSL、Fixed-core Spilter、Random-extra Spilter 与 Efficiency-aware Spilter 等消融版本，用于专门分析尺度分发策略和系统效率。

\subsection{训练设置}

每轮训练中，服务器首先根据客户端尺度分数和系统成本生成尺度计划，再向客户端分发对应尺度参数。客户端本地训练若干 epoch 后只上传被训练尺度，服务器按式~\eqref{eq:scale-agg} 完成尺度级聚合。模型主干采用多尺度 shapelet 编码器，距离度量可设为混合形式；优化器采用 SGD；温度参数、损失系数、客户端数量、通信轮数、本地 epoch 数、批大小等均由配置文件统一控制。若启用 Spilter，应在超参数表中单独列出每客户端激活尺度数 $m$、额外补发尺度数、覆盖惩罚 $\lambda_{\mathrm{cov}}$ 以及通信/计算成本权重 $\lambda_b,\lambda_f$。正式实验中应给出完整超参数表，并说明不同数据集是否采用统一设置或轻度调参。
...
总体实验应重点比较以下方法：集中式 TimeCSL/CSL、FedAvg/FedCSL 全尺度训练、Fixed-core Spilter、Random-extra Spilter 和 Efficiency-aware Spilter。预期结果是，集中式方法在数据可集中时仍然具有较强上界表现；全尺度联邦训练通常具有较强表示能力但通信与计算开销较高；Efficiency-aware Spilter 在只训练 $m$ 个尺度的条件下，通过周期效用和系统成本联合分发，力求接近全尺度性能，同时显著降低每轮通信量与本地计算量。
...
\subsection{尺度冗余性与基础尺度有效性}

为验证式~\eqref{eq:mi-multiscale} 所述的尺度冗余和基础尺度互补性，可在集中式或服务器侧评估环境下分别训练单尺度 shapelet 编码器，提取测试集表示后计算尺度间余弦相似度、互信息估计或 CKA 相似度。若相邻尺度相似度显著高于短尺度与长周期尺度之间的相似度，则说明全尺度模型存在边际信息冗余，而基础尺度选择能够以较少尺度覆盖更互补的频段。
...
为进一步验证“少量互补尺度即可逼近全尺度性能”的结论，可固定联邦划分和训练预算，逐步增加每客户端激活尺度数，并记录下游精度、参数量、FLOPs、通信量和相对全尺度精度。若精度在 $m=4$ 或 $m=5$ 附近进入平台期，而计算和通信继续显著上升，则支持尺度收益的边际递减假设；若子尺度设置在强 non-IID 下超过全尺度训练，则可进一步说明 Spilter 具有结构化正则化效果。
...
\begin{figure}[htbp]
    ...
    \caption{不同 Dirichlet 参数 $\alpha$ 下各尺度分发策略的测试准确率随 round 变化曲线（4$\times$2 子图：自上而下、自左而右依次对应从强异构到弱异构的 $\alpha$ 取值）。}
    \label{fig:dirichlet}
\end{figure}
通过改变 Dirichlet 参数 $\alpha$（对应图~\ref{fig:dirichlet}），可以分析各方法在不同异构程度下的表现。当 $\alpha$ 较小时，客户端数据分布差异更大，此时若 Spilter 和局部--全局蒸馏机制依然能保持较好性能，则说明本文方法对联邦异构性具有较强鲁棒性。此外，还应给出 round 维度上的测试准确率曲线、平均损失曲线、每轮通信量和估计计算量，以展示尺度分发与多尺度蒸馏对收敛速度和训练稳定性的影响。

\subsection{异构系统下的真实时间效率}

除以通信轮数为横轴的收敛曲线外，还应在异构设备或模拟异构速度下报告真实挂钟时间。具体可比较 Full-scale FedCSL、Random-extra Spilter、Fixed-core Spilter 与 Efficiency-aware Spilter 在相同目标精度下的 time-to-accuracy，以及固定训练时长内可达到的最终精度。该实验直接对应式~\eqref{eq:round-makespan} 的 min--max 时延分析：全尺度训练通常受慢客户端拖累，而效率感知分发应通过降低慢客户端负载缩短每轮等待时间。
...
\subsection{尺度分发参数 $m$ 的精度--效率平衡分析}

Spilter 尺度分发策略的核心参数是每客户端激活尺度数 $m$。$m$ 越大，客户端训练更多尺度子模型，表示能力更强但通信和计算开销更高；$m$ 越小，分片通信和本地计算越少，但可能遗漏关键周期尺度而损失精度。本节通过实验分析 $m$ 的变化如何同时影响下游分类精度与系统效率，以验证效率感知补发在精度--效率平衡中的优势。
...
\begin{table*}[htbp]
  \centering
  \caption{Spilter 尺度分发参数 $m$ 对精度与系统效率的影响（$R=6$，Dirichlet $\alpha=0.1$）。Acc.\ 为 SVM 下游测试精度；Comm.\ Ratio 与 Comp.\ Ratio 分别为每轮通信量和计算量占全尺度训练的比例；$\Delta_{\mathrm{upper}}$ 为式~\eqref{eq:spilter-score} 对应的理论精度损失上界估计。}
  \label{tab:m_balance}
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{llcccccc}
    \toprule
...
\begin{enumerate}[leftmargin=2.5em]
    \item \textbf{精度随 $m$ 单调递增，但边际收益递减。} 当 $m$ 从 $2$ 增加到 $4$ 时，三个数据集的精度提升最为显著（HAR: +6.6\%, Epilepsy: +5.5\%, LSST: +3.4\%）；而从 $m{=}4$ 到 $m{=}6$（全尺度），提升幅度明显缩小（HAR: +0.8\%, Epilepsy: +1.7\%, LSST: +0.6\%）。这说明大部分判别性信息已集中在前几个高效用尺度中，后半部分尺度对总体精度的贡献有限。
    \item \textbf{效率感知补发在相同 $m$ 下始终优于随机补发。} 以 HAR 为例，$m{=}2$ 时效率感知比随机补发高出 2.2\%，$m{=}3$ 时高出 2.2\%，$m{=}4$ 时高出 1.7\%。差异随 $m$ 增大而缩小，因为两者最终都趋向全尺度性能。这验证了式~\eqref{eq:spilter-score} 中 $\Phi_{k,r}$ 的贪心选择确实优先激活了单位成本收益更高的尺度。
    \item \textbf{效率感知补发进一步降低实际计算比例。} 表中 Comp.\ Ratio (eff.-aware) 行显示，由于效率感知策略倾向于补发参数量更少、滑窗更短的轻量尺度，$m{=}4$ 时的实际计算占比仅为 0.61，低于均匀成本假设下的 0.67，相当于额外节省约 9\% 的计算量。通信量同样受益于长尺度被延后激活，尽管表中以 $m/R$ 近似表示。
...
尽管本文提出了 Spilter 尺度分发与联邦 shapelet 表征学习的联合框架，但仍存在若干局限。第一，效率感知补发目前使用参数量和滑窗计算量作为系统成本代理，尚未显式纳入真实设备上的动态负载、带宽抖动和能耗约束\cite{oort,fedcsadaptive,flexfl}。第二，本文当前实验重点仍在分类任务，聚类与异常检测等下游任务还需进一步扩展\cite{ts2vec,timescsl}。第三，尺度计划目前主要在训练前基于本地数据统计预计算，未来可进一步设计随轮次更新的在线计划，使尺度分发更好地响应模型训练阶段和客户端状态变化。

\section{结论}

本文围绕联邦非 IID 多变量时间序列场景中的“表示学习”与“尺度分发”两个核心问题，提出了 FedCSL-Spilter 框架。该方法以 TimeCSL/CSL 的多尺度 shapelet 表征学习为骨干，在联邦环境下进一步引入 Spilter 尺度拆分通信、效率感知额外尺度补发、局部--全局联合对比与知识蒸馏、多尺度结构对齐以及周期感知加权机制。理论分析表明，所提方法能够从特征层面抑制客户端漂移，并从系统层面降低尺度冗余带来的通信和计算开销；实验设计则从总体性能、消融实验、非 IID 鲁棒性和可解释性等多个角度验证其有效性。未来工作将继续完善在线尺度计划与真实设备资源建模，并将该框架进一步扩展到联邦异常检测、联邦聚类和个性化联邦学习等更广泛场景中。
