...
def train(dataset="ERing", seed=42, T=0.1, l=1e-2, ls=1.0, alpha=0.5, batch_size=8, to_cuda=True,
    ...
    #X_fed,y_fed = federated_data_split_XD_non_iid(X_all,y_all,num_parties=numClient,ratio=[2,2,6]) # 分出来得到二维dataframe ， 每一个表项 里面有N列，T个时间戳，由逗号分隔 , 需要拿出来作为新的维度，譬如每个客户端拥有的dataframe（12 ，4）需要变成（12，4，100）的nparray
    ...
    client_idc = dirichlet_split_noniid(train_labels=y_all,alpha=0.1,n_clients=numClient)
    print(client_idc[0])
    ...
    #logTxt = "./result/"+dataset+"_fedsc_zero.txt"
    logTxt = f"./result/UEA_l={l}_lr={lr}_epoch{numEpoch}_fedavg_xiugai.txt"
    dataset = "Epilepsy"
