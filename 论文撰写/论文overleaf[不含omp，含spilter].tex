...
\subsection{Spilter 尺度分发的效率与误差分析}

Spilter 可以视为对全尺度目标的结构化子采样。设第 $r$ 个尺度在客户端 $k$ 上诱导的局部损失为 $\mathcal{L}_{k,r}$，则全尺度训练对应
\begin{equation}
\mathcal{L}_{k}^{\mathrm{full}}=\sum_{r=1}^{R}\alpha_{k,r}\mathcal{L}_{k,r},
\end{equation}
而 Spilter 仅在 $\mathcal{R}_k$ 上优化
\begin{equation}
\mathcal{L}_{k}^{\mathrm{split}}=\sum_{r\in\mathcal{R}_k}\tilde{\alpha}_{k,r}\mathcal{L}_{k,r}.
\end{equation}
若 $\mathcal{R}_k$ 覆盖了客户端主要周期结构对应的高权重尺度，则两者差异可由未覆盖尺度的权重和控制。假设每个尺度损失有界 $0\leq\mathcal{L}_{k,r}\leq L_{\max}$，则有
\begin{equation}
...
\end{equation}
其中 $\Delta_{\mathrm{renorm}}$ 表示对已选尺度重新归一化带来的误差。周期感知基础尺度用于减小高权重尺度遗漏概率，而式~\eqref{eq:spilter-score} 中的覆盖惩罚用于避免全局模型长期缺少某些尺度的更新。

从通信角度看，Spilter 的优势直接来自尺度分片。定义通信加速倍率
\begin{equation}
G_{\mathcal{C}}(t)=
...
\end{equation}
因此，在默认半量激活设置下，理论通信加速约为 $R/m$ 倍，等价于每轮双向通信量约下降一半。若长尺度参数量更大，效率感知补发通过 $\kappa_r$ 惩罚高成本尺度，可使实际通信倍率进一步高于均匀成本下的 $R/m$。FlexFL 的经验也表明，在异构联邦中，按资源生成或裁剪子模型可以显著降低通信与训练时间，而关键在于避免粗粒度、固定比例裁剪带来的性能损失\cite{flexfl,scalefl}。本文的尺度分发是在 shapelet 结构上的对应版本：不是裁剪通道或层，而是在多尺度词典中选择更适合当前客户端和系统预算的尺度子模型。

从计算角度看，尺度 $r$ 的前向代价近似正比于 $M_rC\ell_r(T-\ell_r+1)$。同样定义计算加速倍率
\begin{equation}
G_{\mathcal{F}}(t)=
...
\end{equation}
其中本地 epoch 因子在比值中抵消。若各尺度计算成本均匀，则 $G_{\mathcal{F}}(t)\approx R/m$；若长尺度更重，而 Spilter 倾向于补发单位收益--成本比更高的尺度，则 $\sum_{r\in\mathcal{R}_k}F_r$ 会低于随机补发的期望成本
\begin{equation}
\mathbb{E}\left[\sum_{r\in\mathcal{R}_k^{\mathrm{rand}}}F_r\right],
\end{equation}
从而得到更大的 $G_{\mathcal{F}}(t)$。因此，效率感知补发等价于在有限本地计算预算下优先保留单位成本收益更高的尺度；它并不保证全局最优，但提供了一个与系统目标一致的贪心近似。当尺度效用估计较可靠、且各尺度收益满足边际递减趋势时，该策略比随机补发更容易获得较高的收益--成本比。
