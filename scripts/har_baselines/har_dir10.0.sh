#!/usr/bin/env bash
# HAR × Dirichlet α = 10.0 × {fedavg, fedprox, scaffold, fedproto, fedcsl}
#
# 自动在空闲 GPU 上并行派发，核心调度逻辑见 _common.sh + gpu_sched.py。
# 关键约束：2 号卡禁用；空闲 GPU ≥ 2 才挂新任务；预留至少 1 张空闲。
#
# 用法:
#   bash scripts/har_baselines/har_dir10.0.sh
#
# 常用环境变量（都可选）:
#   HAR_MIN_FREE=2          启动门槛（空闲 GPU 数下限），默认 2（至少保留 1 张空闲）
