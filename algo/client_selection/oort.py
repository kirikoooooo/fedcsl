# -*- coding: utf-8 -*-
"""Oort 风格客户端选择器 (OSDI'21 兼容实现)。

内部的 ``OortTrainingSelector`` 与 ``_make_oort_args`` 从根目录的
``oort_selector.py`` 迁移而来，根目录文件保留作为向后兼容 shim。
``OortSelector`` 是统一接口 ``ClientSelector`` 的适配器。
"""
...
class OortTrainingSelector:
    """Oort 训练侧选择器：UCB + 探索 + Pacer。接口与原 ``oort_selector.py`` 等价。"""

    def __init__(self, args=None, sample_seed: int = 233):
        if args is None:
        ...
        self.exploitUtilHistory: List[float] = []
        self.exploreUtilHistory: List[float] = []
        self.exploitClients: List[int] = []
        self.exploreClients: List[int] = []
    ...
    def select_participant(self, num_of_clients, feasible_clients=None):
        viable = (
            feasible_clients
            if feasible_clients is not None
            else set(k for k, v in self.totalArms.items() if v["status"])
        )
    ...
    def _get_top_k(self, numOfSamples, cur_time, feasible_clients):
        ...
        if self.round_threshold < 100.0:
            ...
            idx = min(
                int(len(sorted_duration) * self.round_threshold / 100.0),
                len(sorted_duration) - 1,
            )
        ...
        if n_pick >= len(pickedClients):
            ...
            self.exploitClients = list(
                np.random.choice(pickedClients, size=n_pick, replace=False, p=probs)
            )
...
def create_oort_selector(config=None, sample_seed: int = 233) -> OortTrainingSelector:
    cfg = config or {}
    oort_cfg = cfg.get("oort", {}) if isinstance(cfg, dict) else {}
    args = _make_oort_args(
...
class OortSelector(ClientSelector):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.inner = create_oort_selector(config=config, sample_seed=seed or 42)
        for idx in range(num_clients):
            size = len(y_fed[idx]) if y_fed is not None and idx < len(y_fed) else 0
            reward = min(size, num_epoch * batch_size) if size else 1.0
            self.inner.register_client(
                idx,
                {"reward": reward, "duration": 1.0, "time_stamp": 0, "status": True},
            )
    ...
    def on_round_start(
        ...
        **ctx: Any,
        ...
        feasible = set(range(self.num_clients))
        picked = self.inner.select_participant(self.sample_nums, feasible)
        picked_set = set(int(i) for i in picked)
        return [1.0 if i in picked_set else 0.0 for i in range(self.num_clients)]
    ...
    def on_round_end(
        ...
        **ctx: Any,
        ...
        selected = {i for i in range(self.num_clients) if select_mask[i] > 0}
        for idx in selected:
            loss = client_losses[idx] if idx < len(client_losses) else 0.0
            reward = -float(loss) if loss else 0.0
            self.inner.update_client_util(
                idx,
                {"reward": reward, "duration": 1.0, "time_stamp": round_idx, "status": True},
            )
